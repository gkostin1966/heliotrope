...
RSpec.describe EPub::Unmarshaller::Content do
  ...
  xdescribe '#from_rootfile_full_path' do
  ...
  describe '#idref_with_index_from_href' do
    subject { described_class.from_rootfile_full_path(rootfile, full_path) }
    ...
    before do
      allow(rootfile).to receive(:instance_of?).with(EPub::Unmarshaller::Rootfile).and_return(true)
      allow(File).to receive(:open).with('./META-INF/container.xml')
      allow(File).to receive(:open).with(full_path).and_return(content_xml)
      allow(File).to receive(:open).with(toc_path).and_return(toc_xml)
    end

    context 'href_idref' do
      let(:content_xml) do
        <<-XML
        <content>
          <manifest>
      ...
      it { expect(subject.idref_with_index_from_href(href)).to eq [idref, 1] }
      it { expect(subject.idref_with_index_from_href(href)).to eq ('greg') }
    end

    context 'base_href_idref' do
      let(:content_xml) do
        <<-XML
        <content>
          <manifest>
            <item id="toc" href="toc.xhtml" properties="nav" />
            <item id="1" href="Whatever/1.xhtml"/>
          </manifest>
          <spine>
            <itemref idref="1" />
          </spine>
    ...
    context 'Relative' do
      let(:toc_path) { File.join(full_dir, 'Text', toc_href) }
  ...
  xdescribe '#chapter_from_title' do
    subject { described_class.from_rootfile_full_path(rootfile, full_path) }
