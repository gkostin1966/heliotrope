...
describe ModelTreeActor do
  ...
  describe '#destroy' do
    ...
    context 'next_actor.destroy(env) returns true' do
      ...
      context 'error' do
        let(:message) { 'ERROR: ModelTreeActor.destroy error StandardError' }
        ...
        it 'logs error, does NOT unlink model, but allows stack to continue' do
          is_expected.to be true
          expect(model_tree_service).not_to have_received(:unlink_parent).with(noid)
          expect(model_tree_service).not_to have_received(:unlink_children).with(noid)
