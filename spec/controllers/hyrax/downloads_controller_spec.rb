...
RSpec.describe Hyrax::DownloadsController, type: :controller do
  ...
  describe "#show" do
    ...
    context 'closed caption' do
      ...
      let(:file_set) { create(:public_file_set, user: user, allow_download: 'no', read_groups: ['public'], captions: [captions]) }
      let(:captions) do
        <<~CAPTIONS_VTT
        closed caption a
        closed caption b
    ...
    context 'visual description' do
      ...
      let(:file_set) { create(:public_file_set, user: user, allow_download: 'no', read_groups: ['public'], descriptions: [descriptions]) }
      let(:descriptions) do
        <<~DESCRIPTIONS_VTT
      ...
      it 'downloads visual description metadata' do
        ...
        expect(response).to have_http_status(:ok)
        expect(response.body).to eq descriptions
      end
    end
  end
