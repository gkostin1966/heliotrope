  ...
  context 'job' do
      ...
      context 'when monographs' do
        ...
        before do
          allow(ActiveFedora::SolrService).to receive(:query)
            .with("+has_model_ssim:monograph_doc AND +visibility_ssi:open AND -suppressed_bsi:true",
    ...
    describe '#file_set_docs' do
      ...
      context 'when file set docs' do
        ...
        before do
          allow(ActiveFedora::SolrService).to receive(:query)
            .with("+has_model_ssim:FileSet AND +monograph_doc_id_ssim:#{monograph_doc['id']}",
                  fl: %w[id date_modified_dtsi has_model_ssim monograph_doc_id_ssim],
                  rows: 100_000)
