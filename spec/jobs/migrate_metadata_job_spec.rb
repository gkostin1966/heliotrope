      ...
      context 'file set' do
        ...
        context 'transcript' do
          ...
          it 'migrates' do
            is_expected.to be true
            file_set.reload
            expect(file_set.captions).to eq(['transcript'])
          end

          context 'captions' do
            let(:file_set) { create(:public_file_set, transcript: 'transcript', captions: ['captions']) }

            it do
              is_expected.to be false
              file_set.reload
              expect(file_set.captions).to eq(['captions'])
            end
          end
        end
      end
